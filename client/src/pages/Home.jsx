...
import { ArrowRight, Star, Truck, Shield, Gem, Heart, Sparkles, ChevronRight } from "lucide-react";
    ...
    <div className="bg-[#0A0A0A]">
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-[-10%] right-[-5%] w-[600px] h-[600px] rounded-full bg-gradient-to-br from-[#D4A853]/8 to-[#D4A5A5]/5 blur-3xl" />
          <div className="absolute bottom-[-10%] left-[-5%] w-[500px] h-[500px] rounded-full bg-gradient-to-tr from-[#D4A853]/5 to-[#D4A5A5]/8 blur-3xl" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full bg-[#D4A853]/3 blur-[150px]" />
        </div>

        <div className="nayamo-container relative z-10 py-16 md:py-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#D4A853]/10 border border-[#D4A853]/20 text-[#D4A853] text-sm font-medium mb-6">
                <Sparkles className="w-4 h-4" />
                New Earring Collection 2025
              </div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif font-bold leading-[1.1] mb-6 text-white">
                Trendy <br />
                <span className="nayamo-text-gold">Fashion</span> <br />
                <span className="nayamo-text-rose">Jewellery</span>
              </h1>
              <p className="text-[#9CA3AF] text-lg md:text-xl mb-8 max-w-lg leading-relaxed">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="relative">
              <div className="relative aspect-[4/5] max-w-lg mx-auto rounded-3xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.8)] border border-white/[0.06]">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.5 }} className="absolute bottom-8 left-8 nayamo-glass rounded-2xl px-5 py-3 shadow-lg">
                <p className="text-xs text-[#9CA3AF] uppercase tracking-wider">Starting from</p>
                <p className="text-xl font-bold nayamo-text-gold">Rs 199</p>
              </motion.div>
            </motion.div>
          </div>
        </div>
      ...
      {/* ===== TRUST BADGES ===== */}
      <section className="py-10 bg-[#0F0F0F] border-y border-white/[0.06]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={i} {...fadeInUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-[#D4A853]/10 border border-[#D4A853]/10 flex items-center justify-center flex-shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div key={cat.name} {...fadeInUp} transition={{ duration: 0.5, delay: i * 0.15 }}>
                <Link to={`/shop?category=${cat.name.toLowerCase().replace(" ", "-")}`} className="group relative block aspect-[3/4] rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-500 border border-white/[0.06]">
      ...
      <section className="py-20 bg-[#0A0A0A]">
        <div className="nayamo-container">
          ...
          {loading ? <SkeletonGrid count={4} /> : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {bestSellers.map((product, i) => <ProductCard key={product._id} product={product} index={i} />)}
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-20 bg-[#0A0A0A] text-white relative overflow-hidden">
        ...
        <div className="nayamo-container relative z-10">
          <motion.div {...fadeInUp} className="text-center mb-14">
            ...
            <h2 className="text-3xl md:text-4xl font-serif font-semibold mt-2 text-white">What Our Customers Say</h2>
            <p className="text-[#9CA3AF] mt-2">Real stories from fashion jewellery lovers</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={i} {...fadeInUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="nayamo-card p-8 hover:bg-[#242428] transition-colors">
                <div className="flex gap-1 mb-4">
                  {Array.from({ length: t.rating }).map((_, j) => <Star key={j} className="w-4 h-4 fill-[#D4A853] text-[#D4A853]" />)}
                </div>
                <p className="text-[#E8E8E8] text-sm leading-relaxed mb-6">&ldquo;{t.text}&rdquo;</p>
      ...
      <section className="py-20 bg-[#0F0F0F]">
        <div className="nayamo-container">
          <motion.div {...fadeInUp} className="nayamo-card p-10 md:p-14 text-center relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-lg mx-auto">
              <div className="w-14 h-14 rounded-2xl bg-[#D4A853]/10 border border-[#D4A853]/20 flex items-center justify-center mx-auto mb-5">
                <Heart className="w-7 h-7 text-[#D4A5A5]" />
              </div>
              <h2 className="text-2xl md:text-3xl font-serif font-semibold text-white mb-3">Join the Nayamo Club</h2>
              <p className="text-[#9CA3AF] mb-8 max-w-md mx-auto">Subscribe for exclusive collections, styling tips, and special offers.</p>
              <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" onSubmit={(e) => e.preventDefault()}>
                <input type="email" placeholder="Enter your email" className="nayamo-input flex-1" />
                <button type="submit" className="nayamo-btn-rose whitespace-nowrap">Subscribe</button>
              </form>
            </div>
          </motion.div>
        </div>
